---
title: "Pathway Analysis for FABIA/MOFA groups of interest"
author: "John Zhuang"
date: "2024-08-09"
...
```{r}
results.fm = readRDS("data/sel_features/fabia_mofa_overlap_of_interest.rds")
sel.features = results.fm
sel.features
```
...
```{r}
geneNames <- list("b1f1.1"= multiomics.to.GENES(sel.features$b1f1.1$var),
                  "b1f2.10"= multiomics.to.GENES(sel.features$b1f2.10$var)
                  )
```
```{r}
geneNames
...
all(geneNames$b1f1.1 == geneNames$b1f2.10)
```


```{r}
saveRDS(geneNames, file = "data/sel_features/gene_converted/genes_fabia_mofa_linear.rds")
```
...
saveRDS(pathway, file = "data/out_pathway/pathways_fabia_mofa_linear.rds")
```
